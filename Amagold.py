...
class AMAGOLD(nn.Module):
    def __init__(self, trainloader, mh_trainloader, datasize, T, weight_decay, beta, criterion,temparature,mh_correction):
        ...
        self.mh_correction=mh_correction
    ...
    def update_params(self,model,t,p_buf,lr):
        ...
        for p in model.parameters():
            if t == 0:
                ...
                p_buf[i] = ((1 - self.beta) * p_buf[i] - lr * d_p + (lr*self.beta*self.temparature)**.5*2*eps)/(1+self.beta)
    ...
    def leapfrog(self,model,model_old,p_buf,lr):
        ...
        for batch_idx, (data, target) in enumerate(self.mh_trainloader):
            ...
            model.zero_grad()
            output = model(data)
            U_new = self.criterion(output, target)/self.temparature
            model_old.zero_grad()
            output = model_old(data)
            U_old = self.criterion(output, target)/self.temparature
            break
